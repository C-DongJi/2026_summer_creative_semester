# GPU 세션 ① Runbook (Colab용) — 베이스라인 OOM 테스트 + 청크 로직 실증

> 이 세션은 **5060 Ti PC에서 진행하는 것이 기본**이며, 그 절차는
> [TESTING.md](TESTING.md) 2단계에 있다 (베이스라인 OOM 23.63GB는 이미 실증됨 —
> [SCHEDULE.md](SCHEDULE.md) 참고). 이 문서는 Colab T4로 대신할 때의 절차다.
>
...
| 항목 | 추정 | 근거 |
|---|---|---|
| 모델 가중치 (Kim, fp32) | ~0.9 GB | 2.28억 파라미터 × 4B |
...
- [ ] 보컬/반주가 실제로 분리되는가
- [ ] **청크 경계에서 틱/뚝 소리가 없는가** (8초 청크, 75% 오버랩 → 경계는 2초 간격)
      ← Overlap-Add 크로스페이드 검증 포인트
